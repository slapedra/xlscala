...
namespace xlscala {
	...
	XLRef::XLRef(const std::shared_ptr<Xloper>& xloper)
	: xloper_(xloper) {
		static XLOPER12 xlInt;
		xlInt.xltype = xltypeInt;
		xlInt.val.w = xltypeRef;
		OPER* xlRef = new OPER;
		Excel12(xlCoerce, xlRef, 2, &(*xloper_), &xlInt);
	...
	: xloper_(new XLRefOper()) {
		...
		col_ = col;
		cols_ = cols;
		(&(*(xloper_.get())))->val.mref.lpmref = new XLMREF12;
	...
	bool XLRef::inter(const std::shared_ptr<XLRef>& xlRef) const {
